...
noreturn void handle_connection(int sock)
{
    ...
    int port2 = server_port;
    int lsock2;
    do {
        port2--;
        lsock2 = creer_serveur_tcp(port2, 1, debug);
    } while (lsock2 < 0 && port2 > 1024);
    die_if(lsock2 < 0, "échec de l'ouverture du deuxième port");
    msg("Deuxième port du serveur: %d", port2);

    /* Envoi au client du numéro du deuxième port. */
    die_if(write(sock, &port2, sizeof port2) != sizeof port2,
           "échec de l'envoi du numéro du deuxième port au client");
    close(sock),
